...
class ContadorLimitado extends StatefulWidget {
  ...
  final int          limite;
  final Color        color;          // parÃ¡metro extra para demostrar widget.param
  final VoidCallback? onLimite;      // callback opcional â€” se llama al alcanzar el lÃ­mite
...
class _ContadorLimitadoState extends State<ContadorLimitado> {
  ...
  void _incrementar() {
    ...
    if (_valor >= widget.limite) {
      widget.onLimite?.call();              // notifica al padre si registrÃ³ un callback
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final progreso  = _valor / widget.limite;   // 0.0 â†’ 1.0

    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(widget.etiqueta,                               // â† widget.etiqueta
        ...
        LinearProgressIndicator(
          value:           progreso,
          color:           enLimite ? widget.color : widget.color,   // â† widget.color
        ...
        Text(
          '$_valor / ${widget.limite}',                      // â† widget.limite
          style: TextStyle(
            fontSize:   28,
